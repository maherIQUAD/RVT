...
def log_metrics(cfg, train_or_test, epoch, loss, top1, top5):
    root_output_dir = Path(cfg.OUTPUT_DIR)
    dataset = cfg.DATASET.DATASET
    cfg_name = cfg.NAME
    final_output_dir = root_output_dir / dataset / cfg_name
    log_file = f"{final_output_dir}/{train_or_test}.log"

    with open(log_file, "a") as f:
        f.write(f"mode:{train_or_test},epoch:{epoch},loss:{loss},top1:{top1},top5:{top5}\n")
